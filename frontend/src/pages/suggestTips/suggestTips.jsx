...
import videoLinks from './course.json';  // Import file JSON
...
const SuggestTips = () => {
    ...
    useEffect(() => {
        ...
        setResumeVideos(videoLinks.resumeVideos);
        setInterviewVideos(videoLinks.interviewVideos);
    }, []);
    const videoRowStyle = {
        display: 'flex',
    ...
    return (
        <div className="suggestPage">
            ...
            <div className="course-section">
                    ...
                    <table className="course-table">
                        <tbody>
                            {ds_course.map((course, index) => (
                                <tr key={index}>
                                    <td><a href={course[1]} target="_blank" rel="noopener noreferrer">{course[0]}</a></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                {/* Các khóa học Android, iOS, UI/UX có thể làm tương tự */}
            </div>
        </div>
    );
}
