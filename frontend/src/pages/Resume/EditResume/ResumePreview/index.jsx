...
import { AiFillAppstore } from "react-icons/ai";
import axios from "axios";
import { msMYIntl } from "@ant-design/pro-components";

const ResumePreview = () => {
  const { data, selectedColor, selectedTemplate, templates } =
    useContext(DataContext);
  if ((!templates || templates.length === 0) && (!data || data.length === 0)) {
  ...
  const renderTemplate = () => {
    ...
    if (selectedTemplate) {
      ...
      nameSelected = template?.name;
    } else {
      nameSelected = data?.template?.name;
    }

    switch (nameSelected) {
      case "General":
        return <General data={data} selectedColor={selectedColor} />;
      case "Normal":
        return <Normal data={data} selectedColor={selectedColor} />;
      default:
        return <General data={data} selectedColor={selectedColor} />; // Mặc định là template General
    }
  };
